...
OUT = Path(__file__).resolve().parent.parent / "outputs" / "wsN24"
SEEDS = list(range(16, 32))  # 增量；与 s16 合并为 32 种子
...
def main():
        ...
        for sd in SEEDS:
            ...
            if args.test:
                ...
                pv.append(WSA.predict(model, enc, mean, std, h.m_tr,
                                      device="cuda"))
            print(f"[fuse3-e150s32] seed={sd} ({time.time()-t0:.0f}s)",
                  flush=True)
            del model
    ...
    if args.test:
        pt_old = np.load("outputs/wsN24/pred_test.npy")
        pt = pt + [pt_old] * 16  # 旧16种子等权
